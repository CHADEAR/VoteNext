const dns = require("dns").promises;
const https = require("https");

const EMAIL_REGEX = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

function checkFormat(email) {
  if (!email || typeof email !== "string") return false;
  const trimmed = email.trim().toLowerCase();
  return EMAIL_REGEX.test(trimmed);
}

async function checkMx(email) {
  const domain = email.split("@")[1];
  if (!domain) return false;
  try {
    const records = await dns.resolveMx(domain);
    return Array.isArray(records) && records.length > 0;
  } catch {
    return false;
  }
}

async function checkHunter(email, apiKey) {
  const hunterKey = apiKey || process.env.HUNTER_API_KEY;

  if (!hunterKey) {
    console.warn("HUNTER_API_KEY not set, skipping Hunter check for development");
    console.warn("Email verification will proceed with format and MX checks only");
    return true;
  }

  console.log(`[Hunter] Verifying email: ${email}`);

  const encoded = encodeURIComponent(email);
  const url = `https://api.hunter.io/v2/email-verifier?email=${encoded}&api_key=${hunterKey}`;

  let body;
  try {
    body = await new Promise((resolve, reject) => {
      const req = https.get(url, (res) => {
        let data = "";
        res.on("data", (chunk) => (data += chunk));
        res.on("end", () => resolve(data));
      });

      req.on("error", reject);
      req.on("timeout", () => reject(new Error("Request timeout")));
      req.setTimeout(10000);
    });
  } catch (err) {
    console.warn("[Hunter] request error", err.message);
    throw new Error("Email verification failed: unable to reach Hunter");
  }

  try {
    const json = JSON.parse(body);
    console.log(`[Hunter] Response for ${email}:`, json);

    if (json?.errors) {
      console.warn("[Hunter] API error for", email, json.errors);
      const errMsg =
        json.errors?.[0]?.message ||
        json.errors?.message ||
        JSON.stringify(json.errors);
      throw new Error(`Email verification failed: Hunter returned "${errMsg}"`);
    }

    const data = json?.data ?? {};
    const status = data.status;
    const disposable = data.disposable === true;
    const acceptAll = data.accept_all === true;
    const block = data.block === true;

    console.log(
      `[Hunter] Email ${email} - Status: ${status}, Disposable: ${disposable}, Accept All: ${acceptAll}`
    );

    if (block) {
      throw new Error("This email address is blocked by Hunter");
    }
    if (disposable) {
      throw new Error("Disposable email addresses are not allowed");
    }
    if (acceptAll) {
      throw new Error("This domain cannot be verified because it accepts all mail");
    }
    if (status !== "valid") {
      const reason =
        status === "invalid"
          ? "This email address is invalid or does not exist"
          : status === "unknown"
            ? "This email address could not be verified"
            : `Email verification failed with status: ${status || "unknown"}`;
      throw new Error(reason);
    }

    console.log(`[Hunter] Email ${email} verified successfully`);
    return true;
  } catch (e) {
    if (e.message && e.message.startsWith("Email verification failed")) {
      throw e;
    }
    console.warn("[Hunter] parse error", e.message, "body length:", body?.length);
    throw new Error("Email verification failed");
  }
}

module.exports = {
  checkFormat,
  checkMx,
  checkHunter,
};
